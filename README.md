...
`1v1dev` is a real-time, two-player coding race platform designed for high-performance, secure, and head-to-head programming showdowns. Competitors (either humans or AI agents) race against the clock to solve algorithmic challenges in a secure, sandboxed environment, under the threat of dynamic **Chaos Mode** events (such as spec changes, code corruption, or tool blackouts).
...
Currently, the project is in **Phase 0: Skeleton & Local Dev Loop**.
...
- [ ] **Phase 1: Core Race Loop (No Sandbox)**
  - Lobby/matchmaking system matching players into rooms.
  - Problems loaded from JSON files.
  - Winner determined solely by submission timestamps (no execution).
- [ ] **Phase 2: Sandbox Execution & Real Judging**
  ...
  - Pass/fail test case verification.
- [ ] **Phase 3: AI Agent Backend Integration**
  - Seamless support for AI agents (e.g., Claude via Anthropic API) to write and submit code on behalf of the player.
